...
short* CWaveformImgOperation::CalculateWaveform(BYTE *buffer, int width, int height){
	...
	float scaleValue = (float)width / WAVEFORM_WIDTH;
    unsigned int value;
    short scaledWidth;
...
BYTE* CWaveformImgOperation::GetWaveform(BYTE *imgColor, int imgWidth, int imgHeight, float scale, int ch){
    int val;
    short *lumaData = CalculateWaveform(imgColor, imgWidth, imgHeight);

    BYTE *waveData = m_WaveformBuffer;

	scale = (1080/(float)imgHeight) * 2 * scale;

    for (int i = 0; i < WAVEFORM_HEIGHT; i++){
        for (int j = 0; j < WAVEFORM_WIDTH; j++){
            val = lumaData[i * WAVEFORM_WIDTH + j] * scale;
			if(val >= 0xff) {
				     if(ch == 0) { *waveData++ = 0xFF; *waveData++ = 0x96; *waveData++ = 0x00; }
				else if(ch == 1) { *waveData++ = 0x00; *waveData++ = 0xFF; *waveData++ = 0x00; }
				else if(ch == 2) { *waveData++ = 0x00; *waveData++ = 0x00; *waveData++ = 0xFF; }
				else if(ch == 3) { *waveData++ = 0xFF; *waveData++ = 0xFF; *waveData++ = 0xFF; }
            }
            else{ // frequency * brightness
				switch(ch) {
				case 0: // Blue
					*waveData++ = val * 1;    // B : 255
					*waveData++ = val * 0.58; // G : 150
					*waveData++ = 0;          // R : 0
					break;
				case 1: // Green
					*waveData++ = 0; // B
...
BYTE* CWaveformImgOperation::GetVectorScope(BYTE *imgCb, BYTE *imgCr, int imgWidth, int imgHeight, int scale){
	...
	//scale = (1080/(float)imgHeight) * 3 * scale;
