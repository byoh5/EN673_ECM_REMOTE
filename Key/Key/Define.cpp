...
int TxCtrlKey(int iKey)
{
	...
	do{
		...
		iCnt++;
	} while (((UINT32)flag & 0X0000ffff) == 0xd002);
#else
	BYTE * flag = NULL;
	UINT32 key = 0;
	switch (iKey)
	...
	state = setDataFromRemote(ConnectSocket, KEYADDR, 4, WBCOM, &flag);
#endif
	return state;
}
...
int WriteComm(BYTE *pBuff, int nToWrite)
{
	...
	state = setDataFromRemote(ConnectSocket, JTAG_UART_ADDR, 12, WBCOM, &n_temp);
	if (state < 0)	return -1;
