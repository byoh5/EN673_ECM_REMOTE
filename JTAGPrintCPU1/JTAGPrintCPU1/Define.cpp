...
UINT32 JTAGPrint_define(UINT8* pbuf, UINT32 cpu)
{
	...
	TRACE("JTAGPrint_define 2\r\n");
	// get size of jtag_print
	//if (err |= pJTAG->read32(jtag_print_point_addr, now_pos, 1, JTAG_COMMON_MODULE_IDX))	goto END;
	if (-1 == (err = getDataFromRemote(ConnectSocket, jtag_print_point_addr, 4, JTAG_COMMON_MODULE_IDX, now_pos)))	goto END;

	if (now_pos == NULL) { TRACE("now_pos == NULL\r\n");  goto END; }

	TRACE("JTAGPrint_define 3\r\n");
	*now_pos = jtag_htonl((UINT32)(*now_pos));
	...
	TRACE("JTAGPrint_define 8\r\n");
	uiReturn = size;
END:
	m_csJTAGPrint.Unlock(); // Critical Section Lock
	TRACE("JTAGPrint_define Unlock\r\n");

	if (err != 0)
	{
		NetClo(ConnectSocket);
		ConnectSocket = NULL;
		pJTAGPrintCPU1Dlg->m_bRunJTAGPrint = TRUE;
		//pJTAGPrintCPU1Dlg->OnBnClickedButtonJtagprintcpu1Start();
	}
...
UINT32 ReadJTAGPrintData(UINT32 last_pos, UINT32 size, UINT8* pbuf, UINT32 cpu)
{
	...
	for (i = 0; i < retry; i++){
		...
		if (-1 == (err = getDataFromRemote(ConnectSocket, re_last_pos + (MR_READBUFFER * i), MR_READBUFFER, JTAG_COMMON_MODULE_IDX, (UINT32*)(memblock + (MR_READBUFFER * i))))) {
			goto END;
		}
	}
	if (left) {
		//if (err |= pJTAG->read32(last_pos + (MR_READBUFFER * i), (UINT32*)(memblock + (MR_READBUFFER * i)), (left / 4) + 1, JTAG_COMMON_MODULE_IDX)) {
		if (-1 == (err = getDataFromRemote(ConnectSocket, re_last_pos + (MR_READBUFFER * i), left + 1, JTAG_COMMON_MODULE_IDX, (UINT32*)(memblock + (MR_READBUFFER * i))))) {
			goto END;
		}
	}
	TRACE("ReadJTAGPrintData getDataFromRemote \r\n");
...
void AutoSave()
{
	...
	{
		fileTxt.SeekToEnd();
		pJTAGPrintCPU1Dlg->m_Edit_String.GetWindowTextW(data);
		//data.Delete(data.GetLength() - 1, 1);
		for (int i = 0; i < pJTAGPrintCPU1Dlg->m_iMaxLine*REMOVE_RATE; i++) {
			data.Delete(data.GetLength() - 1, 1);
