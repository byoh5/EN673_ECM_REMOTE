...
#define 	TVAX_NONE			(TVAE_NONE   >>TVAE_MODEPOS)// 수정하지 않음
#define 	TVAX_EDIT			(TVAE_EDIT	 >>TVAE_MODEPOS)// 에디트박스로 자동으로 편집
#define 	TVAX_COMBO			(TVAE_COMBO	 >>TVAE_MODEPOS)// 콤보박스로 자동으로 편집
...
typedef struct
{
	void 	   *pCbData;									// Daten f? das Autoeditieren
	...
	BYTE  	    bDummy[32-20-sizeof(void*)];				// F?lbytes auf 32 Grenze
} ColumnData;
...
typedef struct
{
	...
	int			iMaxSizeX;									// Die Gr秤e des breitesten sichtbaren Eintrages
	...
	unsigned	uTreeItemsMax;								// Gr秤e der Liste mit den vorhanden Eintr?e (alociert um 1 gr秤er)
	...
	unsigned	uUserDataSize;								// Ist die Gr秤e der Userdaten in einem Eintrag
	unsigned	uFirstChild;								// Ist die Nummer des ersten Kind-Eintrages (0=keines)
	unsigned	uLastChild;									// Ist die Nummer des letzten Kind-Eintrages (0=keines)
	...
	unsigned	uSizeX;										// Aktuelle X-Fenster-Gr秤e
	unsigned	uSizeY;										// Aktuelle Y-Fenster-Gr秤e
	unsigned	uSizeYsub;									// Aktuelle Y-Fenster-Gr秤e ohne Header
	...
	unsigned	uToolTipSize;								// Textspeichergr秤e f? Tooltip
...
//*
//*****************************************************************************
//	WM_NOTIFY 메세지와 부모 윈도우를 보냄.
//	pData		: 데이터 윈도우에 대한 포인터
//	pNotify		: 알림 데이터 포인터
//  WM_NOTIFY 메시지의 반환 값이 있는 경우 (Ergibt den R?kgabewert der WM_NOTIFY Nachrich)
...
//*
//*****************************************************************************
//	훞dert die Gr秤e der variablen Spalten
//	pData		: Zeiger auf die Fensterdaten
//	iDelta		: Ist die Gr秤en?derung in Pixel
...
static HIMAGELIST CreateDragImage(TreeListData *pData,unsigned uItem,unsigned uSub)
{
	...
	if(pData->hImages && iImage>=0)							// Gr秤en f? Images anpassen
	{
...
static void UpdateColors(TreeListData *pData)
{
	...
	if(!pData->cColorChanged[TVC_FRAME  ])pData->uColors[TVC_FRAME  ]  = RGB(215,215,215);
...
//	pData		: Zeiger auf die Fensterdaten
//	Ergibt 1 bei 훞derungen sonst 0
...
RECT	sRect;
	...
	*/
	if(pData->hChecks)
	if(iHeight<pData->iChecksYsize+4)iHeight=pData->iChecksYsize+4;
	if(pData->hStates)
	if(iHeight<pData->iStatesYsize+4)iHeight=pData->iStatesYsize+4;
	if(iHeight<pData->iSubImgYsize+4)iHeight=pData->iSubImgYsize+4;
	if(iHeight<pData->iImagesYsize+4)iHeight=pData->iImagesYsize+4;
	if(iHeight<pData->iFontHeight +4)iHeight=pData->iFontHeight +4;
	if(pData->uStyleEx&TVS_EX_ITEMLINES)iHeight++;
	if(pData->uStyle  &TVS_NONEVENHEIGHT && (iHeight&1))iHeight++;
	if(pData->iRowHeight==iHeight)return 0;
	...
	if(uFlags&TVHT_ONITEM)
	{
		if(pData->uToolTipItem!=uItem || pData->uToolTipSub!=0)
		{
			...
			if(sRect.right-sRect.left<=pEntry->iTextPixels+4)
			{
				...
				if(uLen>=pData->uToolTipSize)				// Tooltipspeicher vergr秤ern
					{
...
static int UpdateColumns(TreeListData *pData)
{
	...
	for(uPos=0;uPos<pData->uColumnCount;)					// Suche die erste ge?derte Spalte
	{
...
//	iMode		: Ist der Grund f? die 훞derung
//						TVC_BYKEYBOARD
//						TVC_BYMOUSE
//						TVC_UNKNOWN
...
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Eintrag f? den Focus (0xFFFFFFFF=keine 훞derung)
//	uSub		: Spalte f? den Focus	(0xFFFFFFFF=keine 훞derung)
...
//*****************************************************************************
	...
	pTemp	= pData->pTreeItems[uOld ];						// Zeiger neu holen falls es 훞derungen gab
	pEntry	= pData->pTreeItems[uItem];
...
int			iShift;
	...
	if(pData->uTreeItemsCount+1>pData->uTreeItemsMax)		// Gr秤e der Liste erh?en
		{
...
//*
//*****************************************************************************
//	훞dert einen Eintrag im Fenster
//	pData		: Zeiger auf die Fensterdaten
...
unsigned		uSub;
	...
	if(pFind->uFlags&TVIF_CHILD)							// In den Kindern suchen
		...
		if(pFind->uFlags&TVIF_NEXT)							// Beim n?hsten Eintrag weitersuchen
			{
				pEntry = pData->pTreeItems[uItem];
			if(!pEntry)return 0;
...
//*
//*****************************************************************************
// 마우스 클릭을 해제함. 잠금 해제를 실행한다는 것에 주의하셈.
//	pData	: 데이터 윈도우에 대한 포인터.
//	uMsg	: 마우스 클릭 메시지
//	wParam	: 마우스 클릭 wParam
//	lParam	: 마우스 클릭 lParam
static void TreeListMouseClick(TreeListData *pData,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	if(!pData->cIsEnabled)									// 윈도우가 잠겨있음.
	{
		return;
	}

	if(!pData->cHasFocus)									// 윈도우에 포커스가 있는 경우
	{
		UNLOCK(pData);
		SetFocus(pData->hWnd);
		LOCK(pData);
	...
	if(uItem)												// Wurde auf einen Eintrag getr?kt
	{
		...
		if(sInfo.flags&TVHT_ONITEMBUTTON)					// 엔트리 오픈
		{
		...
		else if(sInfo.flags&TVHT_ONITEMSTATEICON)			// 토글 확인란
		{
			if(pData->uStyle&TVS_CHECKBOXES)
			if(uMsg==WM_LBUTTONDOWN || uMsg==WM_LBUTTONDBLCLK)
			{
			...
			if(pData->uStyleEx&TVS_EX_MULTISELECT)			// 다중 선택 허용
				{
			...
			if(uMsg==WM_LBUTTONDBLCLK) // 일반 더블클릭 처리
				{
...
char			cCb;
	...
	if(cCb)													// Callback aufrufen
		...
		HWND hWnd = ::GetParent(pData->hWnd);
		if (pParameterDlg->m_hWnd == hWnd)
			::PostMessage(pParameterDlg->m_hWnd, WM_PAR_ENDLABELEDIT, 0, 0);
...
HDC				hDc;
	...
	case WM_SIZE:												// Die Fenstergr秤e wurde ver?dert
			...
			if(uVal && uVal!=pData->uSizeX)
				...
				if(uVal>uOld)									// Hat sich die Breite vergr秤ert
					{
					RECT	sRect;
			...
			if(uVal && uVal!=pData->uSizeY)
				{
				if(uVal>pData->uSizeY)							// Hat sich die H?e vergr秤ert
					{
					RECT	sRect;
				...
				if(uFlag && pData->uPageEnties>2)				// Wenn H?e vergr秤ert dann Scroll-Position pr?en
					{
			...
			if(wParam==1)
				...
				if(pHdr->hdr.code==HDN_TRACK || pHdr->hdr.code==HDN_ENDTRACK)
					...
					if(pData->uStyleEx&TVS_EX_FIXEDCOLSIZE)		// Fixe gesammte Spaltenbreite
						...
						for(iNext=iCol+1;;iNext++)				// ?erspringe fixierte Spalten
							{
							if(U(iNext)>=pData->uColumnCount)
								{
								UNLOCK(pData);
						...
						if(iDelta<0)							// N?hste Spalte wird zu klein
							...
							if(pData->aColumn[iNext].bMinEx)	// Minimale Breite pr?en
	...
	case TVM_SETEXTENDEDSTYLE:									// Einstellen der erweiterten Style-Bits
			...
			if(pData->uStyleEx!=uVal)							// Gab es 훞derungen
				{
				LOCK(pData);
	...
	case TVM_SETUSERDATASIZE:									// Einstellen der Gr秤e der User-Daten
	...
	case TVM_GETUSERDATASIZE:									// Abfragen der Gr秤e der User-Daten
...
//******************** Eintr?e zeichnen **************************************
	...
	for(;uPos<uMax;uPos++)										// Alle Eintr?e ausgeben
	{
		...
		else											// Farbe wechselweise ?dern
		{
			uColMark  =  pData->aColumn[0].bMark;
			uMark     = 0xFFFFFFFF;
			if(uPos&1)	uOutColor = (uColMark)? uOcColor:uOdColor;
			else		uOutColor = (uColMark)? uEcColor:uEvColor;
		}
		...
		{													// Das Feld speziel zeichnen
			...
			if(!pEntry->iTextPixels && uTextSize)
				...
				pEntry->iTextPixels = sButton.right-sButton.left;
																...
																// Ist der Text gr秤er als die Spalte
