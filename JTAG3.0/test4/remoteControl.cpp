...
UINT32 rem_Sfls_erase(char* cmdline);
UINT32 rem_si(char* cmdline);
...
TCLI_CMD cli_cmds[] =
{
	...
	{ "rem_tbp",		rem_tbp,				"tbp"						},
	{ "rem_Sfls_erase", rem_Sfls_erase,			"Sfls_erase"				},
	{ "rem_si",			rem_si,					"single step"				},
	{ 0, 0, 0 }
};
...
DWORD WINAPI threadReadMem_(void *arg)
{
	...
	while (jtag_gdb_burst_read32((UINT*)rbuf, size4 / 4, addr, module)){ TAP_Reset();	Sleep(1); }
...
UINT32 rem_write_mem_(char* cmdline)
{
	...
	for (a = 0; a < size;){
		...
		if (zero_count > 1000)
		{
			rprintf("recv zero 1000 times error\n");
			return 0;
		}
		#endif
	}
...
//	hexDump_("READ", buf, 1024);
	...
	// write to memory
	while (jtag_gdb_burst_write32((UINT*)wbuf, size4 / 4, addr, module)){ TAP_Reset();	Sleep(1); }
...
//		rprintf("Addr %x\n", addr);
		...
		state = send(cli_fd, (char*)sendmsg, strlen(sendmsg), 0);
#if 1
		if (state < 0)
		{
			rprintf("image send error\n");
			break;
		}
#endif
		Sleep(10);
		state = send(cli_fd, (char*)wbuf, size, 0);
#if 1
		if (state < 0)
		{
			rprintf("image send error\n");
...
UINT32 rem_reset_cpu(char* cmdline)
{
	...
	if (sscanf(cmdline, "rem_rc %s %d ", &module, &cli_fd) == 2) {
		if (strcmp("cpu0", module) == 0)
			err |= _DoCommand("rc cpu0");
		else if (strcmp("cpu1", module) == 0)
			err |= _DoCommand("rc cpu1");
		else
		{
			rprintf("Error sendimg arg err!\n");
			return -1;
...
UINT32 rem_unreset_cpu(char* cmdline)
{
	...
	if (sscanf(cmdline, "rem_urc %s %d ", &module, &cli_fd) == 2) {
		if (strcmp("cpu0", module) == 0)
			err |= _DoCommand("urc cpu0");
		else if (strcmp("cpu1", module) == 0)
			err |= _DoCommand("urc cpu1");
		else
		{
			rprintf("Error sendimg arg err!\n");
			return -1;
...
UINT32 rem_stall(char* cmdline)
{
	...
	if (sscanf(cmdline, "rem_st %s %d ", &module, &cli_fd) == 2) {
		if (strcmp("cpu0", module) == 0)
			err |= _DoCommand("st cpu0");
		else if (strcmp("cpu1", module) == 0)
			err |= _DoCommand("st cpu1");
		else
		{
			rprintf("Error sendimg arg err!\n");
			return -1;
...
UINT32 rem_unstall(char* cmdline)
{
	...
	if (sscanf(cmdline, "rem_ust %s %d ", &module, &cli_fd) == 2) {
		if (strcmp("cpu0", module) == 0)
			err |= _DoCommand("ust cpu0");
		else if (strcmp("cpu1", module) == 0)
			err |= _DoCommand("ust cpu1");
		else
		{
			rprintf("Error sendimg arg err!\n");
			return -1;
...
UINT32 rem_Sfls_erase(char* cmdline)
{
	...
	if (sscanf(cmdline, "rem_Sfls_erase %x %s %d ", &addr, &module, &cli_fd) == 3) {
		if (strcmp("all", module) == 0)
			err |= Sfls_erase_all();
		else if (strcmp("block", module) == 0)
			err |= Sfls_erase_block(addr);
		else if (strcmp("sect", module) == 0)
			err |= Sfls_erase_sect(addr);
		else
		{
			rprintf("Error sendimg arg err!\n");
			return -1;
...
UINT32 rem_si(char* cmdline)
{
	...
	char module[10];
	int cli_fd;
	if (sscanf(cmdline, "rem_si %s %d ", &module, &cli_fd) == 2) {
		if (strcmp("cpu0", module) == 0)
			gcpu = 4;
		else if (strcmp("cpu1", module) == 0)
			gcpu = 5;
	...
	send(cli_fd, ((char*)buffer), sizeof(buffer), 0);
