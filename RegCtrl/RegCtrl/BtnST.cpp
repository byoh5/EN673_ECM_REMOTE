...
CButtonST::CButtonST()
{
	...
	// ADD
	m_bReverseBg = FALSE;
	m_ReverseBgColor = GetSysColor(COLOR_BTNFACE);

	m_bTracking = FALSE;
	m_bHover = FALSE;

	m_bPressedShadow = TRUE;
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bIsPressed)
	{
		...
		else    
		{
			...
			//CBrush brBtnShadow(MYCOLORSHADOW);
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		*/
		...
		if( (captionRect.left - m_iRightX) > 0) {
			captionRect.OffsetRect(-m_iRightX, 0);
		}

		// Center text
		CRect centerRect = captionRect;
		pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
		captionRect.OffsetRect((centerRect.Width() - captionRect.Width())>>1, (centerRect.Height() - captionRect.Height())>>1);
		/* RFU
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if(bIsDisabled)
	{
		...
		pDC->DrawState(	CPoint(rImage.left-4/*+1*/, rImage.top), 
	...
	{
		::BitBlt(pDC->m_hDC, rImage.left-4, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcMem, 0, 0, SRCAND);
		::BitBlt(pDC->m_hDC, rImage.left-4, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
	}
...
void CButtonST::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	...
	if( (rpImage->left + m_iLeftX) > 0)
		rpImage->OffsetRect(m_iLeftX, 0);
...
{
	// 버튼 수정
	m_crColors[BTNST_COLOR_BK_IN]	= ::GetSysColor(COLOR_3DHILIGHT);
	m_crColors[BTNST_COLOR_FG_IN]	= ::GetSysColor(COLOR_BTNTEXT);//::GetSysColor(COLOR_HIGHLIGHT);
	m_crColors[BTNST_COLOR_BK_OUT]	= ::GetSysColor(COLOR_BTNFACE);
	m_crColors[BTNST_COLOR_FG_OUT]	= ::GetSysColor(COLOR_BTNTEXT);
	//m_crColors[BTNST_COLOR_BK_IN]	= MYCOLORBK;
	//m_crColors[BTNST_COLOR_FG_IN]	= MYCOLORTEXT;
	//m_crColors[BTNST_COLOR_BK_OUT]	= MYCOLORBK;
	//m_crColors[BTNST_COLOR_FG_OUT]	= MYCOLORTEXT;
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	...
	if(m_bReverseBg)
	{
		...
		CBrush	brSubRectangle(m_ReverseBgColor);
		pDC->FillRect(rect, &brSubRectangle);
	}
...
void CButtonST::SetReverseBg(BOOL bFlag)
{
	m_bReverseBg = bFlag;
}

void CButtonST::SetReverseBgColor(COLORREF color)
{
	m_ReverseBgColor = color;
}
...
void CButtonST::SetDrawPressedShadow(BOOL bFlag)
{
	m_bPressedShadow = FALSE;
}